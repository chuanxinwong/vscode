 ...
 */
async function getHtmlContent(fileService: IFileService): Promise<string> {
	try {
		const htmlUri = FileAccess.asFileUri('vs/workbench/contrib/myApp/browser/media/index.html');
		const result = await fileService.readFile(htmlUri);
		let html = result.value.toString();

		// Convert media folder to webview-safe URI and rewrite relative src/href
		const mediaUri = asWebviewUri(FileAccess.asFileUri('vs/workbench/contrib/myApp/browser/media'));
		const baseUrl = mediaUri.toString(true) + '/';
		html = html.replace(/(href|src)="(?!https?:\/\/)([^"]+)"/g, (_m, attr, path) => {
			return attr + '="' + baseUrl + path + '"';
		});

		// Replace CSP placeholders
		html = html.replace(/\{\{CSP_SOURCE\}\}/g, webviewGenericCspSource);

		return html;
	} catch {
		return '<!DOCTYPE html><html><body><h1>MyApp</h1><p>Failed to load.</p></body></html>';
	}
}

CommandsRegistry.registerCommand(MyAppOpenCommandId, async (accessor) => {
	...
	const webviewInput = webviewWorkbenchService.openWebview(
		{
			...
			contentOptions: { allowScripts: true, localResourceRoots: [FileAccess.asFileUri('vs/workbench/contrib/myApp/browser/media')] },
			extension: undefined,
		},
		'myApp.editor',
	...
	webviewInput.webview.setHtml(await getHtmlContent(fileService));
